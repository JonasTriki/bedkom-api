...
const presentationsBodyValidator = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const { companyId, startTime, endTime, responsible, menuId } = req.body;
    ...
    const students = responsible.map(
      async (uid: string) => await UserModel.get(uid)
    );
