...
toByteString =
  encodeUtf8 .
  toLazyText . htmlEncodedText . L.toStrict . decodeUtf8 . renderTags
...
fetchHiddenFields sess = do
  ...
  let extraRe = [re|mojarra\.jsfcljs\(.*{'(.*)':'(.*)'}.*\)|]
      tags =
        map nameValuePair $ filter (~== "<input type=hidden>") feideModuleBox
      feideModuleBox =
        takeWhile (~/= "</section>") .
        dropWhile (~/= "<section data-flap-name=feide>") . parseTags $
        getRes ^. responseBody
