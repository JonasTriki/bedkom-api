...
const onlyInformaticsStudies = (program: string) =>
  studyPrograms.indexOf(program) > -1;
...
const parseProfileDetails = ($: CheerioStatic): ProfileDetails | null => {
  try {
    ...
    const profileWrapper = $('p:contains("studentnummer:")')
      .text()
      .trim()
      .split("\n");
    ...
    const email = $("div[class=korr-epost]")
      .text()
      .trim();
...
const parseContainers = ($: CheerioStatic): Study[] => {
  ...
  return containers
    ...
    .map(container => {
      ...
      const startSemester =
        params.get("termin").toLowerCase() === "høst" ? "spring" : "autumn";
...
const scrapStudent = async (
  ...
  const feideModuleBoxHtml = loginResParsed(
    "section[data-flap-name=feide]"
  ).html();
  ...
  const currentStudiesParsed = cheerio.load(currentStudies, {
...
export const fetchInformaticsStudent = (
  username: string,
  password: string,
  org: string
): Promise<Student | null> => {
